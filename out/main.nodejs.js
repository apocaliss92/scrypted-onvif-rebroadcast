        ...
        if (!onvifIp && this.plugin.storageSettings.values.autoIpEnabled) {
            ...
            if (baseIp) {
                ...
                const rtspTargets = this.discoveredStreams
                    ...
                    .filter((t) => t !== null);
                const shimIp = this.plugin.storageSettings.values.macvlanShimIp || undefined;
...
class IpAliasManager {
     ...
     */
    ensureMacvlanShim(parentIface, ip, prefix, override) {
        if (this.shimIp !== null)
            return this.shimIp;
        let shimIp;
        if (override) {
        ...
        try {
            const addrOut = (0, child_process_1.execSync)(`ip addr show ${iface} 2>/dev/null || true`).toString();
            ...
            if (addrOut.trim()) {
                (0, child_process_1.execSync)(`ip link del ${iface} 2>/dev/null || true`);
            }
            (0, child_process_1.execSync)(`ip link add ${iface} link ${parentIface} type macvlan mode bridge`);
            (0, child_process_1.execSync)(`ip addr add ${shimIp}/${prefix} dev ${iface}`);
            (0, child_process_1.execSync)(`ip link set ${iface} up`);
            this.shimIp = shimIp;
            ...
            this.console.error(`Failed to create macvlan shim interface (${msg}). ` +
    ...
    removeShim() {
        if (!this.shimIp)
            return;
        try {
            (0, child_process_1.execSync)(`ip link del ${IpAliasManager.SHIM_IFACE} 2>/dev/null || true`);
    ...
    async addAlias(deviceId, ip, parentIface, prefix, gatewayOverride, rtspTargets, shimIpOverride) {
        ...
        if (this.needsMacvlanShim(scryptedIp, ip, prefix)) {
            if (this.shimIp) {
                ...
                this.console.warn(`Scrypted IP ${scryptedIp} is on the macvlan subnet — shim creation failed. ` +
    ...
    constructor(nativeId) {
        ...
        this.storageSettings = new storage_settings_1.StorageSettings(this, {
            ...
            macvlanShimIp: {
                ...
                description: "IP address for the macvlan shim interface created on the host when Scrypted runs outside Docker. " +
